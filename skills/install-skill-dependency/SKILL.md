...
2. **Detect if running in a VM/secure workspace**: Check indicators such as:
   - Existence of `/root/{{.DataDirName}}` (common in containers/VMs)
...
1. **Locate skills directory**: Check these paths in order:
   - macOS / Linux: `~/{{.DataDirName}}/skills/`
   - Linux VM / Container: `/root/{{.DataDirName}}/skills/`
   - Windows: `%USERPROFILE%\{{.DataDirName}}\skills\`
...
- **VM/secure workspace environment**: You may proceed more freely, but still ask before installing.
