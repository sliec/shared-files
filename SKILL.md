...
Claude 风格的核心不是某种花哨效果，而是一种**克制、温暖、有文档质感的审美**。三个关键词：

1. **克制（Restraint）**：少即是多。不堆叠效果、不滥用颜色、不强调一切（强调一切等于没强调）。
2. **温暖（Warmth）**：用暖色调（米白、陶土橙），避免冰冷的纯白和科技蓝。给人"纸张"和"手账"的亲切感。
3. **可读（Readability）**：充足留白、清晰层级、舒服的行高。页面是给人"认真读"的，不是"快速划过"的。
...
1. **Start with the template**: Read [template.html](template.html) for the full base structure
...
- [ ] 背景用了暖米白，没用纯白 `#FFFFFF` 大面积铺底
- [ ] 正文用近黑暖调 `#1F1E1C`，没用纯黑 `#000`
- [ ] 陶土橙只用在关键强调处，没有滥用
- [ ] 大标题用衬线体，正文/UI用无衬线体，代码用等宽体
- [ ] 行高 1.6 左右，正文宽度不超过 ~680px
- [ ] 留白充足，区块间距 >= 48px
- [ ] 圆角 8–12px，边框很淡，阴影极轻
- [ ] 一个区域只有一个主按钮（陶土橙）
- [ ] 所有交互有 0.15–0.25s 过渡，曲线非线性
- [ ] 没有荧光色、重渐变、夸张动效
- [ ] 用了语义化标签，有 focus 状态
- [ ] 加了 `prefers-reduced-motion` 处理
- [ ] 是单个自包含 .html 文件（样式内联在 `<style>` 里）
...
The complete working template with all components is at:
[template.html](template.html)

Read it whenever you need to verify the exact implementation of any component.
